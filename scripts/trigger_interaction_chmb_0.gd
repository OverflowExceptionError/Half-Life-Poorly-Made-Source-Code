...
# var a = 2
# var b = "text"
var physics_check = self.get_colliding_bodies()
...
func _process(delta):
	physics_check = self.get_colliding_bodies()
	if physics_check.size() > 0:
		if physics_check[0].name == "Player":
			get_tree().change_scene("maps/chamber2.tscn")
	print(physics_check)
